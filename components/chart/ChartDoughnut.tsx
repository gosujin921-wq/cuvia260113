    ...
    return (
        <div className={fill ? "w-full h-full" : "h-[260px] w-full flex items-center justify-center"}>
            <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                    <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={fill ? "52%" : 68} outerRadius={fill ? "80%" : 105} paddingAngle={1}>
                        {pieData.map((_, index) => (
                            <Cell key={`cell-${index}`} fill={DOUGHNUT_COLORS[index % DOUGHNUT_COLORS.length]} stroke="rgba(30,30,35,0.95)" strokeWidth={1} />
                        ))}
                    </Pie>
                    <Tooltip
                        contentStyle={{

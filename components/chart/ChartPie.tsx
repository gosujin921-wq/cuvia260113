        ...
        <div className={fill ? "w-full h-full flex" : "h-full w-full flex"} style={{ pointerEvents: isRenderEnd ? "auto" : "none" }}>
            <div className="flex-1 min-w-0 flex items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={fill ? "80%" : 105} animationBegin={ANIM_BEGIN} animationDuration={ANIM_DURATION} onMouseEnter={handlePieMouseEnter} onMouseLeave={handlePieMouseLeave}>
                            {pieData.map((_, index) => (
                                <Cell key={`cell-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} stroke="rgba(30,30,35,0.95)" strokeWidth={1} opacity={activeHover === null || activeHover === index ? 1 : LEGEND_DIM_OPACITY} />
                            ))}
                        </Pie>
                        {isRenderEnd && (
                            <Tooltip

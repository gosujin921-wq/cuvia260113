...
const EventList = ({ events, selectedEventId, onEventSelect, onEventHover, key1PressTime }: EventListProps) => {
    ...
    const [priorityFilter, setPriorityFilter] = useState<"ALL" | "긴급" | "경계" | "주의" | "일반">("ALL");
    ...
    const filteredEvents = priorityFilter === "ALL" ? sortedEvents : sortedEvents.filter((event) => event.priority === priorityFilter);

    // 우선순위별 건수 계산
    const getPriorityCount = (priority: "긴급" | "경계" | "주의" | "일반") => {
    ...
    const generalCount = getPriorityCount("일반");
        ...
        <div className="w-full flex flex-col h-full overflow-y-auto">
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            <React.Fragment key={tab.value}>
                                <button onClick={() => setPriorityFilter(tab.value)} className={`pb-2 text-xs font-semibold tracking-tight transition-colors flex items-center ${isActive ? "text-white border-b-2 border-blue-400" : "text-gray-400 border-b-2 border-transparent hover:text-white"}`}>
                                    {getPriorityDot()}
                                    {tab.label}
                                    {tab.count !== null && <span className="ml-1 text-gray-400">({formatCount(tab.count)})</span>}
                                </button>
                                {(tab.value === "ALL" || tab.value === "주의") && <span className="w-1 h-1 rounded-full bg-gray-500 self-center" style={{ marginBottom: "10px" }} />}
                            </React.Fragment>
                        );
                    })}
                </div>
            ...
            <div ref={scrollContainerRef} className="flex-1 overflow-y-auto space-y-2">
                {filteredEvents.length === 0 ? (
                    <div className="text-gray-500 text-xs px-3 py-6 border-b border-[#2f3136]">표시할 이벤트가 없습니다.</div>
                ) : (
                    filteredEvents.map((event) => {
                        ...
                        return (
                            <div
                                ...
                                className={`w-full text-left border-b pt-3 pb-3 pr-3 transition-all duration-200 ${isSelected ? "bg-red-500/10 border-red-500/50 ring-2 ring-red-500/30" : "bg-transparent border-[#2f3136] hover:bg-[#24272d] hover:border-blue-400"}`}
                                ...
                                {/* 이벤트 제목 */}
                                <div className="text-white text-sm font-semibold mb-1.5">{displayTitle}</div>

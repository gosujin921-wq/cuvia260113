        ...
        <div
            ...
            {isCollapsed ? (
                <div className="flex-1 flex flex-col items-center justify-between py-8 pl-4 pr-2 gap-6 text-[0.65rem] text-gray-300">
                    <div className="flex flex-col items-center gap-2 text-[10.4px]">
                        ...
                        {collapsedIndicators.map((indicator) => (
                            <div key={indicator.label} className="flex flex-col items-center gap-1">
                                ...
                                <span className={`${indicator.color} text-sm font-semibold`}>{indicator.value.toLocaleString()}</span>
                            </div>
                        ))}
                    </div>
                    <div className="flex flex-col items-center gap-2 text-[10.4px]">
                    ...
                    <button onClick={handleFacilityClick} className="flex flex-col items-center gap-1 text-center text-[10.4px] focus:outline-none">
                ...
                <div className="flex-1 overflow-hidden pt-4 pb-4 pl-6 pr-6 flex flex-col gap-4" ref={scrollContainerRef} style={{ maxWidth: "100%", height: "100%" }}>
                        ...
                        <div className="space-y-4 mt-3">
                            <div className="grid grid-cols-2 gap-3 min-w-0">
                                {visibleAreas.map((area, areaIndex) => (
                                    <div key={`${area.area}-${areaPage}`} className="bg-[#393a42] px-3 py-2 space-y-3 min-w-0 overflow-hidden rounded-lg">
                            ...
                            <div className="flex justify-end h-full">
                                <div ref={trendChartContainerRef} className="relative flex-1 max-w-[434px] rounded-xl overflow-visible" style={{ minHeight: "80px", width: "100%", height: "100%" }}>
                                    <ResponsiveContainer width="100%" height="100%" minHeight={80}>
                                        <AreaChart data={animatedTrendData} margin={{ top: 26, right: 20, left: 0, bottom: 30 }}>
                                            <defs>
                                                ...
                                                <linearGradient id="lightReflectionStroke" x1="0%" y1="0%" x2="100%" y2="0%" gradientUnits="userSpaceOnUse">
                                                    ...
                                                    <animateTransform attributeName="gradientTransform" type="translate" values="-200 0;400 0" dur="5s" repeatCount="indefinite" />
                                                </linearGradient>
                                            </defs>
                                            <CartesianGrid strokeDasharray="1.5 2" stroke="rgba(107, 114, 128, 0.3)" opacity={1} />
                                            <XAxis dataKey="xValue" type="number" domain={[0, 24]} ticks={X_AXIS_TICKS} tick={renderTrendXAxisTick} tickLine={false} axisLine={{ stroke: "rgba(107, 114, 128, 0.3)", strokeWidth: 1 }} tickMargin={8} />
                                            <YAxis domain={[0, 50]} tick={renderTrendYAxisTick} tickLine={false} axisLine={{ stroke: "rgba(107, 114, 128, 0.3)", strokeWidth: 1 }} ticks={[0, 25, 50]} width={40} allowDecimals={false} />
                                            <Area type="basis" dataKey="value" stroke="url(#eventWaveStroke)" strokeWidth={3} fill="url(#eventWaveFill)" fillOpacity={0.9} isAnimationActive={false} dot={false} activeDot={false} />
                                            {/* 빛 반사 효과 - 선에만 적용 */}
                                            <Area type="basis" dataKey="value" stroke="url(#lightReflectionStroke)" strokeWidth={4} fill="none" opacity={0.7} isAnimationActive={false} />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                        ...
                                        <div
                                            ...
                                            <div className="grid grid-cols-12 gap-1 flex-1 min-w-0">
                                                {heatmapTimeSlots.map((slot, slotIndex) => {
                                                    ...
                                                    const newColorValue = bucket === "low" ? "#3b5a8c" : bucket === "mid" ? "#005eb8" : bucket === "high" ? "#F87171" : "#1f2937";

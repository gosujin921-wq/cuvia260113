...
const AIAgentPopup: React.FC<AIAgentPopupProps> = ({ isOpen, onClose, hideControls = false, eventTime }) => {
        ...
        if (isPositiveResponse(prompt)) {
            ...
            return {
                ...
                analysisResult: {
                    ...
                    evidence: ["서로 밀치며 접촉 동작이 반복 관측됨 (CCTV-V-11 14:33:26~14:33:35)", "1인이 뒤로 밀리며 휘청이는 장면이 관측됨(낙상 위험) (CCTV-V-11 14:33:36~14:33:40)", "주변 인원이 근접했다가 이탈하는 패턴이 관측됨(혼잡 가능) (CCTV-V-11 14:33:41~14:33:55)", "흉기/위험물: 미확인(화면에서 명확히 식별되지 않음)"],
                    recommendations: ["[현장 요원 확인] 진행 중 충돌로 안전사고위험이 있습니다.", "[관련 인물 고속 검색] 관련 인물의 최근 이동 경로를 인근 카메라에서 조회합니다.", "[112 협조 요청(권고)] 상황이 지속·확대되거나 통제 필요 정황이 있을 경우 현장 안전 확보를 위해 협조 요청을 권고합니다."],
                },
            };
        }
        return {
        ...
        <>
            {isExpanded ? (
                            ...
                            <div className="p-4">
                                <div className="relative flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 focus-within:border-blue-500 transition-colors">
                                    ...
                                    <textarea
                                        ...
                                        rows={1}
                                    />
                                    <button type="button" onClick={handleSendMessage} disabled={!chatInput.trim() || isResponding} className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full transition-all disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 active:scale-95 self-center" style={{ background: AGENT_GRADIENT }} aria-label="전송">
                ...
                <div
                    ...
                    <div className="flex flex-col rounded-2xl bg-white border border-gray-200 shadow-lg relative overflow-hidden w-[420px] max-h-[600px]">
                        <div className="absolute top-3 right-3 z-10 flex items-center gap-2">

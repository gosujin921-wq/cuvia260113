...
const MOCK_SUMMARIES: Partial<Record<ImageId, string>> = {
  ...
  '30': '회색 후드 티셔츠와 어두운색 청바지, 흰색 운동화를 착용한 흑색 짧은 머리의 슬림한 체격 인물이 원미A-481 검지1 화각 하단에 등장하여 휴대폰을 손에 든 채 일정한 속도로 도로 중앙을 따라 전방 이동한 후 화면 상단 방향(계남로 방면, 북서 방향)으로 보행을 유지하며 최종 이탈함.',
...
const MOCK_TIMELINES: Partial<Record<ImageId, Omit<TimelineEntry, 'seconds'>[]>> = {
  ...
  '30': [
    ...
    { time: '00:12', label: '화면 상단 방향(계남로 방면)으로 보행을 유지하며 최종 이탈 확인' },
  ],
  '40': [

...
const replaceCctvNameInText = (text: string): string => {
  let result = text;
  for (const [orig, replacement] of Object.entries(ORIGINAL_TO_STARLIGHT)) {
...
const MOCK_SUMMARIES: Partial<Record<ImageId, string>> = {
  ...
  '11': '밝은 회색 또는 흰색 계열의 상의와 검은색 하의, 흰색 운동화를 착용한 슬림한 체격의 인물이 투명한 패턴 우산을 쓴 상태로 사랑교회 인근 이면도로에 등장하여 화면 우측 하단에서 중앙 도로 방향으로 보행을 시작한 후 일정한 속도를 유지하며 카메라 반대 방향(화면 상단)으로 최종 이탈함.',
  '15': '흰색(또는 매우 밝은 회색) 패딩 상의와 검은색 하의, 흰색 운동화를 착용한 인물이 손에 스마트폰을 든 채 사랑교회 인근 사거리에 진입하여 \'소한공인중개사\' 앞 횡단보도를 대각선 방향으로 횡단하며 지속적으로 휴대폰 화면을 응시하는 보행 패턴을 보인 후 화면 하단 왼쪽 방향(남서 방향)으로 보행을 유지하며 최종 이탈함.',
  ...
  '40': '회색 후드 티셔츠와 어두운색 청바지, 흰색 운동화를 착용한 흑색 머리의 슬림한 체격(176cm, 65kg 추정) 인물이 별빛A-498 검지 화각 하단(달빛로301번길 54 인근)에 등장하여 보행 중 손에 든 휴대폰을 응시하며 일방통행 화살표 방향(화면 상단)으로 직선 보행을 유지한 후 화면 상단 방향(북동 방향)으로 최종 이탈함.',
  ...
  '51': '회색 후드 티셔츠와 어두운색 청바지, 흰색 운동화를 착용한 흑색 짧은 머리의 슬림한 체격 인물이 원미A-583 검지3 화각 상단(건강약국 앞 도로)에 등장하여 고개를 숙이고 양손으로 스마트폰을 조작하며 주변 보행자나 차량 흐름을 확인하지 않고 스마트폰 화면에 집중하며 카메라 정면 방향으로 이동한 후 카메라 정면 하단 방향(남서 방향)으로 일정한 보행 속도를 유지하며 최종 이탈함.',
...
const MOCK_TIMELINES: Partial<Record<ImageId, Omit<TimelineEntry, 'seconds'>[]>> = {
  ...
  '15': [
    { time: '00:00', label: '사랑교회 인근 사거리(고정4 화각) 진입 확인 (추정)' },
    { time: '00:02', label: '\'소한공인중개사\' 앞 횡단보도를 대각선 방향으로 횡단하며 휴대폰 조작' },
  ...
  '40': [
    { time: '00:00', label: '별빛A-498 검지 화각 하단(달빛로301번길 54 인근)에 인물 등장 (추정)' },
  ...
  '51': [
    { time: '00:00', label: '원미A-583 검지3 화각 상단(건강약국 앞 도로)에 인물 등장 (추정)' },
...
export const getCandidateDetailData = (
  ...
  const timelineWithReplacedLabels = timeline.map((t) => ({
    ...t,
    label: replaceCctvNameInText(t.label),
  }));
...
export const generateMarkdownAnalysis = (
  ...
  return replaceCctvNameInText(markdown);
};

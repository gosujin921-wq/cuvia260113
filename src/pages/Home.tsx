...
export default function Home() {
        ...
        const handleKeyPress = (e: KeyboardEvent) => {
            ...
            if (e.key === "1") {
                ...
                if (missingEvent) {
                    ...
                    setMissingEventId(missingEvent.id);
                    setTimeout(() => {
                        setMapZoomLevel(2);
                        setTimeout(() => {
    ...
    }, [allConvertedEvents, animateToEvent, missingEventId]);
                ...
                <div className="rounded-lg p-4 gradient-border-right-bottom" style={{ flexShrink: 0, background: "linear-gradient(135deg, rgba(0,0,0,0.6) 0%, rgba(23,23,23,0.6) 100%)", backdropFilter: "blur(2px)", WebkitBackdropFilter: "blur(2px)" }}>
                    <div className="grid grid-cols-2 gap-3">
                        {[
                            ...
                            return (
                                <div key={card.title} className="bg-[#393a42] p-3 rounded-lg flex items-start justify-between relative">
                                    {/* 타이틀과 숫자 */}
                                    <div className="flex flex-col gap-0.5">
                                        <span className="text-gray-400 text-xs font-medium">{card.title}</span>
                                        <div className="text-white text-2xl font-bold">{card.value.toLocaleString()}</div>
                                    </div>
                                    {/* 아이콘 */}

...
const FEATURES = [
  ...
  {
    title: 'CUVIA LINK',
    desc: '도메인지식 기반 의사결정 지원',
    bullets: [
      '자연어 질의 기반 데이터 분석·대응',
      '정리·요약·보고서까지 워크플로우 연결',
      '분석에서 대응까지 실행 중심 Workspace',
    ],
    icon: (
      <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5">
...
const SectionLabel = ({ children }: { children: string }) => (
  <p className="text-gray-500 text-[10px] tracking-[0.2em] uppercase font-semibold text-center mb-3 flex-shrink-0">{children}</p>
);
  ...
  return (
    <div className="flex items-center justify-center w-screen h-screen bg-black overflow-hidden">
      ...
      >
        ...
        <div
          className={`relative z-10 flex flex-col h-full px-7 py-6 transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
        >
          {/* Logo */}
          <div className="flex flex-col items-center flex-shrink-0">
            ...
            />
            <img src="/logo.svg" alt="CUVIA" className="h-4" style={{ filter: 'brightness(0) invert(1)' }} />
          </div>
          ...
          <div className="flex flex-col items-center text-center flex-shrink-0">
            <p className="text-blue-400 text-[10px] font-semibold tracking-[0.3em] uppercase mb-2">
              Next Generation AI Platform
            </p>
            <h1 className="text-white font-bold leading-tight mb-3" style={{ fontSize: '32px', letterSpacing: '-0.02em' }}>
              예측이 가능한 시대
            </h1>
            <p className="text-gray-400 text-[12px] leading-relaxed">
              분산된 도시 데이터를 연결하고 영상 맥락 기반 분석·예측을 통해<br />
              복잡한 도시 운영을 명확한 판단과 신속한 대응으로 전환하는<br />
              지능형 도시 운영 플랫폼입니다.
            </p>
          </div>
          ...
          {/* Search Flow */}
          <SectionLabel>Intelligence Workflow</SectionLabel>
          <div className="flex items-start justify-between gap-1 px-1 flex-shrink-0">
            {SEARCH_FLOW.map((item, i) => (
              <div key={item.step} className="flex flex-col items-center flex-1 relative">
                <div
                  className="w-10 h-10 rounded-full flex flex-col items-center justify-center mb-2"
                  style={{
                    background: 'linear-gradient(135deg, rgba(0,94,184,0.4) 0%, rgba(0,94,184,0.15) 100%)',
                    border: '1px solid rgba(0,94,184,0.4)',
                  }}
                >
                  <span className="text-blue-300/70 text-[6px] font-medium tracking-[0.1em] uppercase leading-none">STEP</span>
                  <span className="text-white text-[14px] font-bold leading-none mt-0.5">{item.step}</span>
                </div>
                {i < SEARCH_FLOW.length - 1 && (
                  <div
                    className="absolute top-[20px] left-[calc(50%+22px)] h-[1px]"
                    style={{ width: 'calc(100% - 44px)', background: 'linear-gradient(90deg, rgba(0,94,184,0.5), rgba(0,94,184,0.15))' }}
                  />
                )}
                <p className="text-white text-[10px] font-semibold text-center leading-tight">{item.label}</p>
                <p className="text-gray-500 text-[8px] text-center mt-0.5">{item.sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-4 gap-2 flex-shrink-0">
            {KEY_CAPABILITIES.map((cap) => (
              <div
                key={cap.label}
                className="relative rounded-lg py-3 text-center gradient-border-right-bottom flex flex-col items-center"
                style={{ background: 'linear-gradient(135deg, rgba(0,94,184,0.08) 0%, rgba(33,96,173,0.04) 100%)' }}
              >
                <div className="text-blue-400 mb-1.5">{cap.icon}</div>
                <p className="text-white text-[10px] font-semibold leading-tight">{cap.label}</p>
                <p className="text-blue-400/70 text-[8px] mt-0.5">{cap.sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 flex-shrink-0">
            {FEATURES.map((feature) => (
              <div
                key={feature.title}
                className="relative rounded-lg overflow-hidden gradient-border-right-bottom flex flex-col"
                style={{
              ...
              >
                <div className="px-3 py-3 flex flex-col flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div
                      className="flex-shrink-0 w-6 h-6 rounded flex items-center justify-center text-blue-400 [&>svg]:w-4 [&>svg]:h-4"
                      style={{
                        background: 'linear-gradient(135deg, rgba(0,94,184,0.2) 0%, rgba(0,94,184,0.08) 100%)',
                    ...
                    <div className="min-w-0 leading-none">
                      <h3 className="text-white text-[11px] font-bold tracking-tight">{feature.title}</h3>
                      <span className="text-blue-400/60 text-[9px] font-medium leading-none">{feature.desc}</span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-1">
                    {feature.bullets.map((bullet) => (
                      <div key={bullet} className="flex items-start gap-1.5">
                        <span className="text-blue-400/50 text-[8px] mt-[2px] flex-shrink-0">✦</span>
                        <p className="text-gray-400 text-[10px] leading-snug">{bullet}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {/* AI Agents */}
          <div className="text-center flex-shrink-0 mb-2 [&>p:first-child]:mb-0.5">
            <SectionLabel>AI Agent</SectionLabel>
            <p className="text-gray-400 text-[10px]"><span className="text-blue-400 font-semibold">CUVIA LINK</span> 기반 지능형 에이전트 · 근거 기반 판단 · 실행 중심 워크플로우</p>
          </div>
          <div className="grid grid-cols-2 gap-2 flex-shrink-0">
            {AI_AGENTS.map((agent) => (
              <div key={agent.name} className="py-3 px-2">
                <h4 className="text-white text-[10px] font-bold mb-2 text-center">{agent.name}</h4>
          ...
          <div className="grid grid-cols-3 gap-1.5 flex-shrink-0">
            {PRODUCT_LINEUP.map((product) => (
              <div
                key={product.name}
                className="relative rounded-lg py-2.5 px-2 text-center gradient-border-right-bottom"
                style={{ background: 'linear-gradient(135deg, rgba(0,94,184,0.08) 0%, rgba(33,96,173,0.04) 100%)' }}
              >
                <p className="text-white text-[9px] font-bold leading-tight">{product.name}</p>
                <p className="text-gray-400 text-[8px] mt-0.5">{product.desc}</p>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-center gap-8 flex-shrink-0">
            {STATS.map((stat) => (
              <div key={stat.label} className="flex flex-col items-center">
                <span className="text-white text-[20px] font-bold tabular-nums">{stat.value}</span>
          ...
          <div className="text-center flex-shrink-0">
            <p className="text-white/90 text-[13px] font-semibold">"CUVIA는 단순한 솔루션이 아닙니다"</p>
            <p className="text-blue-400 text-[12px] font-medium mt-0.5">하나의 지능형 운영 생태계입니다</p>
          </div>
          <div className="flex flex-col items-center gap-1.5 mt-1.5 flex-shrink-0">
            <div className="flex justify-center gap-1.5">
              {DOMAINS.slice(0, 5).map((d) => (
                <span key={d.label} className="inline-flex items-center gap-1 text-gray-400 text-[9px] font-medium px-2 py-1 rounded-full border border-gray-700/50 bg-white/[0.03]">
                  <span className="text-blue-400/70">{d.icon}</span>{d.label}
                </span>
              ))}
            </div>
            <div className="flex justify-center gap-1.5">
              {DOMAINS.slice(5).map((d) => (
                <span key={d.label} className="inline-flex items-center gap-1 text-gray-400 text-[9px] font-medium px-2 py-1 rounded-full border border-gray-700/50 bg-white/[0.03]">
                  <span className="text-blue-400/70">{d.icon}</span>{d.label}
                </span>
              ))}
          ...
          <div className="flex flex-col items-center flex-shrink-0 mt-auto">
            <div
              className="w-full rounded-lg px-5 py-3 text-center mb-3 gradient-border-right-bottom"

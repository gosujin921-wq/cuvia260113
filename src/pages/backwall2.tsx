...
const DISPLAY_WIDTH = 1255;
const DISPLAY_HEIGHT = 1792;
...
export default function Backwall2Page() {
  ...
  return (
    <div className="flex items-center justify-center w-screen h-screen bg-black overflow-hidden">
      ...
      >
        <GlowOrb className="w-[500px] h-[500px] bg-blue-600 -top-32 -left-32" animIndex={0} />
        <GlowOrb className="w-[400px] h-[400px] bg-indigo-500 top-[500px] -right-20" animIndex={1} />
        <GlowOrb className="w-[450px] h-[450px] bg-blue-500 top-[1000px] -left-32" animIndex={2} />
        <GlowOrb className="w-[350px] h-[350px] bg-purple-600 bottom-[300px] right-0" animIndex={3} />
        ...
        <div
          className={`relative z-10 flex flex-col h-full px-16 py-10 transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
        >
          {/* Logo */}
          <div className="flex flex-col items-center flex-shrink-0">
            <img src="/logo.svg" alt="CUVIA" className="h-14" style={{ filter: 'brightness(0) invert(1)' }} />
          </div>
          ...
          <div className="flex flex-col items-center text-center flex-shrink-0">
            <p className="text-blue-400 text-[28px] font-semibold tracking-[0.3em] uppercase mb-5">
              Next Generation AI Platform
            </p>
            <h1 className="text-white font-bold leading-tight mb-5" style={{ fontSize: '88px', letterSpacing: '-0.02em' }}>
              스마트시티의 미래,<br />지능형 운영을 재정의하다
            </h1>
            <p className="text-gray-300 text-[32px] leading-relaxed">
          ...
          <div className="flex flex-col flex-1 gap-14">
            ...
            <div>
              ...
              <div className="flex items-start justify-between">
                {KEY_CAPABILITIES.map((cap) => (
                  <div key={cap.label} className="flex flex-col items-center flex-1">
                    <div
                      className="rounded-full flex items-center justify-center mb-4 text-blue-400 [&>svg]:w-14 [&>svg]:h-14"
                      style={{
                        width: '110px',
                        height: '110px',
                    ...
                    <p className="text-white text-[32px] font-bold text-center leading-snug whitespace-pre-line">{cap.label}</p>
                    <p className="text-gray-300 text-[24px] text-center mt-2">{cap.sub}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-6">
                {FEATURES.map((feature) => (
                  <div
                    key={feature.title}
                    className="relative rounded-2xl overflow-hidden gradient-border-right-bottom"
                    style={{
                  ...
                  >
                    <div className="px-8 py-7">
                      <div className="flex items-start gap-5 mb-5">
                        <div
                          className="flex-shrink-0 w-20 h-20 rounded-xl flex items-center justify-center text-blue-400 [&>svg]:w-10 [&>svg]:h-10"
                          style={{
                        ...
                        <div className="min-w-0 pt-1">
                          <h3 className="text-white text-[32px] font-bold tracking-wide leading-tight">{feature.title}</h3>
                          <p className="text-blue-300 text-[26px] font-medium mt-1.5">{feature.desc}</p>
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-2.5">
                        {feature.keywords.map((kw) => (
                          <span key={kw} className="text-gray-300 text-[22px] px-5 py-2 rounded-lg border border-gray-500/50 bg-white/[0.08]">{kw}</span>
                        ))}
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex flex-col items-center mt-auto">
              <img src="/cudo_CI.svg" alt="CUDO Communications" className="h-20 opacity-50" style={{ filter: 'brightness(0) invert(1) opacity(0.6)' }} />
            </div>
          </div>
        </div>
      </div>
